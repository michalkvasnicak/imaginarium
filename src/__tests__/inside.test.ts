...
describe('resize (inside)', () => {
  describe('resize to width', () => {
    it('does not support this type', async () => {
      await wrap(server.get('/test-file-name/inside/w100').expect(404));
    });
  });

  describe('resize to height', () => {
    it('does not support this type', async () => {
      await wrap(server.get('/test-file-name/inside/h100').expect(404));
    });
  });
