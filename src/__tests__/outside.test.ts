...
import app from '../app';
import { jpegBigFixture, pngFixture } from './fixtures';
import { expectResponse, wrap } from './helpers';

const getObject: jest.Mock = getObjectMock;
const server = request(app);

describe('resize (outside)', () => {
  describe('resize to width', () => {
    it('does not support this type', async () => {
      await wrap(server.get('/test-file-name/outside/w100').expect(404));
    });
  });

  describe('resize to height', () => {
    it('does not support this type', async () => {
      await wrap(server.get('/test-file-name/outside/h100').expect(404));
    });
  });
  ...
  describe('resize to width and height', () => {
    it('resizes jpeg', async () => {
      ...
      await expectResponse(
        server,
        '/test-file-name/outside/100x100',
        'image/png',
        200,
        true,
      );
